...
class Nmf:
	...
	def factorize(self, dataset=self.data, fnum=10, maxiter=50):
		...
		#initialize weights and features
		...
		for i in range(maxiter):
			...
			#calculate current error btwn
			#dataset matrix and wf
			#matrix
			cost = self.difCost(dataset, wf)
			#if i is divisble by 10 print
			#this error value
			if i % 10 == 0:
				print(cost)
				
			#terminate if matrix has been
			#fully factorized
			if cost == 0:
				break
		...
		#after maxiter iterations or full
		#factorization return the 
		#weights and features matrix 
		#objects 
